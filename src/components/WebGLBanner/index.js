...
const WebGLBanner = ({ minLines, maxLines }) => {
  ...
  useEffect(() => {
    ...
    const numLines = getRandomNumberBetween(minLines, maxLines)
    const colorPalette =
    ...
    function getRandomEdgePoint() {
      ...
      switch (edge) {
        case 0:
          return [-1, Math.random() * 2 - 1] // Left edge
        case 1:
          return [1, Math.random() * 2 - 1] // Right edge
        case 2:
          return [Math.random() * 2 - 1, -1] // Bottom edge
        case 3:
          return [Math.random() * 2 - 1, 1] // Top edge
        default:
          return [0, 0]
      }
    }
    ...
    function render() {
      ...
      lines.forEach((line, index) => {
        ...
        gl.lineWidth(
          Math.min(line.weight, gl.getParameter(gl.ALIASED_LINE_WIDTH_RANGE)[1])
        )
        ...
        switch (line.state) {
          case "drawing":
            ...
            if (line.progress === 1) {
              line.state = "waiting"
              line.delay = getRandomNumberBetween(1000, 5000) // Wait 1-5 seconds
            }
            break
